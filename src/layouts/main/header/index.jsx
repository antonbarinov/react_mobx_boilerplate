...
export default function LayoutHeader() {
    ...
    return (
        <header className={styles.header}>
            ...
            <CustomLink to="/page/1?foo=bar" exact>
                Page 1
            </CustomLink>
            <CustomLink to="/page/2?a=b" exact>
                Page 2
            </CustomLink>
            <CustomLink to="/qweqweewq/3">Page not found</CustomLink>
            <CustomLink to="/profile" exact>
                Private route
            </CustomLink>
            {user && (
                <span className={styles.link} onClick={userState.logout}>
                    Logout
                </span>
            )}
        </header>
...
LayoutHeader = observer(LayoutHeader);
...
function CustomLink({ children, to, className, ...rest }) {
    ...
    return (
        <Link to={to} {...rest} activeClass={styles.active} className={stylesStr.join(' ')}>
            {children}
        </Link>
    );
}
